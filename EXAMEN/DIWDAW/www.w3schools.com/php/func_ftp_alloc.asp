...
&lt;?php<br>// connect and login to FTP server<br>
 $ftp_server = &quot;ftp.example.com&quot;;<br>$ftp_conn = ftp_connect($ftp_server) or die(&quot;Could not connect to $ftp_server&quot;);<br>$login = ftp_login($ftp_conn, $ftp_username, $ftp_userpass);<br><br>$file = &quot;/test/myfile&quot;;<br><br>// allocate space<br>if (ftp_alloc($ftp_conn, filesize($file), $result))<br>&nbsp; {<br>&nbsp; echo &quot;Space allocated on server. Sending $file.&quot;;<br>&nbsp; ftp_put($ftp_conn, &quot;/files/myfile&quot;, $file, FTP_BINARY);<br>&nbsp; }<br>else<br>&nbsp; {<br>&nbsp; echo &quot;Error! Server said: $result&quot;;<br>&nbsp; }<br><br>// close connection<br>ftp_close($ftp_conn);<br>?&gt;
</div>
</div>
<hr>
