...
&lt;?php<br>// connect and login to FTP server<br>
 $ftp_server = &quot;ftp.example.com&quot;;<br>$ftp_conn = ftp_connect($ftp_server) or die(&quot;Could not connect to $ftp_server&quot;);<br>$login = ftp_login($ftp_conn, $ftp_username, $ftp_userpass);<br><br>$local_file = &quot;localfile.txt&quot;;<br>$server_file = &quot;serverfile.txt&quot;;<br><br>// initiate upload<br>$d = ftp_nb_put($ftp_conn, $server_file, $local_file, FTP_BINARY)<br><br>while ($d == FTP_MOREDATA)<br>&nbsp; {<br>&nbsp; // do whatever you want<br>&nbsp; // continue uploading<br>&nbsp; $d = ftp_nb_continue($ftp_conn);<br>&nbsp; }<br>
