...
&lt;?php<br>// connect and login to FTP server<br>
 $ftp_server = &quot;ftp.example.com&quot;;<br>$ftp_conn = ftp_connect($ftp_server) or die(&quot;Could not connect to $ftp_server&quot;);<br>$login = ftp_login($ftp_conn, $ftp_username, $ftp_userpass);<br><br>$server_file = &quot;somefile.txt&quot;;<br><br>// open local file to write to<br>$local_file = &quot;local.txt&quot;;<br>$fp = fopen($local_file,&quot;w&quot;);<br><br>// download server file and save it to open local file<br>if (ftp_fget($ftp_conn, $fp, $server_file, FTP_ASCII, 0))<br>&nbsp; {<br>&nbsp; echo &quot;Successfully written to $local_file.&quot;;<br>&nbsp; }<br>else<br>&nbsp; {<br>&nbsp; echo &quot;Error downloading $server_file.&quot;;<br>&nbsp; }<br><br>// close connection and file handler<br>ftp_close($ftp_conn);<br>fclose($fp);<br>?&gt;
</div>
</div>
<hr>
