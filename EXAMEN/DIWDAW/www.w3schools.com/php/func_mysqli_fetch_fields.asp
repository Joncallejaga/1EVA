...
&nbsp;&nbsp;}<br><br>$sql=&quot;SELECT Lastname,Age FROM Persons ORDER BY Lastname&quot;;<br>
 <br>if ($result=mysqli_query($con,$sql))<br>&nbsp; {<br>&nbsp; // Get field information for all fields<br>&nbsp; $fieldinfo=mysqli_fetch_fields($result);<br><br>&nbsp; foreach ($fieldinfo as $val)<br>&nbsp;&nbsp;&nbsp; {<br>&nbsp;&nbsp;&nbsp; printf(&quot;Name: %s\n&quot;,$val-&gt;name);<br>&nbsp;&nbsp;&nbsp; printf(&quot;Table: %s\n&quot;,$val-&gt;table);<br>&nbsp;&nbsp;&nbsp; printf(&quot;max. Len: %d\n&quot;,$val-&gt;max_length);<br>&nbsp;&nbsp;&nbsp; }<br>&nbsp; // Free result set<br>&nbsp; mysqli_free_result($result);<br>}<br>
...
<tr>
 ...
 <td style="width:80%">Returns an array of objects containing field definition information. FALSE if no info is available. The objects have the following properties:
 <ul>
