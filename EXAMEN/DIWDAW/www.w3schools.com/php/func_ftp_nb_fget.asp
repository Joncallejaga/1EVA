...
&lt;?php<br>// connect and login to FTP server<br>
 $ftp_server = &quot;ftp.example.com&quot;;<br>$ftp_conn = ftp_connect($ftp_server) or die(&quot;Could not connect to $ftp_server&quot;);<br>$login = ftp_login($ftp_conn, $ftp_username, $ftp_userpass);<br><br>$server_file = &quot;somefile.txt&quot;;<br><br>// open local file to write to<br>$local_file = &quot;local.txt&quot;;<br>$fp = fopen($local_file,&quot;w&quot;);<br><br><br>// initiate download<br>$d = ftp_nb_fget($ftp_conn, $fp, $server_file, FTP_BINARY)<br><br>while ($d == FTP_MOREDATA)<br>&nbsp; {<br>&nbsp; // do whatever you want<br>&nbsp; // continue downloading<br>&nbsp; $d = ftp_nb_continue($ftp_conn);<br>&nbsp; }<br>
 <br>if ($d != FTP_FINISHED)<br>&nbsp; {<br>&nbsp; echo &quot;Error downloading $server_file&quot;;<br>&nbsp; exit(1);<br>&nbsp; }<br><br>// close connection and file handler<br>ftp_close($ftp_conn);<br>fclose($fp);<br>?&gt;
</div>
</div>
<hr>
