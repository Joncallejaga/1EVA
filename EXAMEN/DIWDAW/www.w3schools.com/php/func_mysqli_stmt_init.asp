...
&nbsp;&nbsp;}<br><br>$city=&quot;Sandnes&quot;;<br><br>// Create a prepared statement<br>
 $stmt=mysqli_stmt_init($con);<br><br>if (mysqli_stmt_prepare($stmt,&quot;SELECT District FROM City WHERE Name=?&quot;))<br>&nbsp; {<br><br>&nbsp; // Bind parameters<br>&nbsp; mysqli_stmt_bind_param($stmt,&quot;s&quot;,$city);<br><br>&nbsp; // Execute query<br>&nbsp; mysqli_stmt_execute($stmt);<br><br>&nbsp; // Bind result variables<br>&nbsp; mysqli_stmt_bind_result($stmt,$district);<br><br>&nbsp; // Fetch value<br>&nbsp; mysqli_stmt_fetch($stmt);<br><br>&nbsp; printf(&quot;%s is in district %s&quot;,$city,$district);<br><br>&nbsp; // Close statement<br>&nbsp; mysqli_stmt_close($stmt);<br>&nbsp; }<br>
