...
&lt;?php<br>// connect and login to FTP server<br>
 $ftp_server = &quot;ftp.example.com&quot;;<br>$ftp_conn = ftp_connect($ftp_server) or die(&quot;Could not connect to $ftp_server&quot;);<br>$login = ftp_login($ftp_conn, $ftp_username, $ftp_userpass);<br><br>// change the current directory to php<br>
 ftp_chdir($ftp_conn, &quot;php&quot;);<br>
 <br>// change to the parent directory<br>if (ftp_cdup($ftp_conn))<br>&nbsp; { <br>&nbsp; echo &quot;Successfully changed to parent directory.&quot;;<br>&nbsp; }<br>
 else<br>&nbsp; {<br>&nbsp; echo &quot;cdup failed.&quot;;<br>&nbsp; }<br><br>// output current directory name<br>echo ftp_pwd($ftp_conn);<br><br>// close connection<br>ftp_close($ftp_conn);<br>?&gt;
</div>
</div>
<hr>
