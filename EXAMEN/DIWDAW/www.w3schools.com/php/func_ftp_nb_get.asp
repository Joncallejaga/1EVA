...
&lt;?php<br>// connect and login to FTP server<br>
 $ftp_server = &quot;ftp.example.com&quot;;<br>$ftp_conn = ftp_connect($ftp_server) or die(&quot;Could not connect to $ftp_server&quot;);<br>$login = ftp_login($ftp_conn, $ftp_username, $ftp_userpass);<br><br>$local_file = &quot;local.zip&quot;;<br>$server_file = &quot;server.zip&quot;;<br><br>// initiate download<br>$d = ftp_nb_get($ftp_conn, $local_file, $server_file, FTP_ASCII)<br><br>while ($d == FTP_MOREDATA)<br>&nbsp; {<br>&nbsp; // do whatever you want<br>&nbsp; // continue downloading<br>&nbsp; $d = ftp_nb_continue($ftp_conn);<br>&nbsp; }<br>
 <br>if ($d != FTP_FINISHED)<br>&nbsp; {<br>&nbsp; echo &quot;Error downloading $server_file&quot;;<br>&nbsp; exit(1);<br>&nbsp; }<br><br>// close connection<br>ftp_close($ftp_conn);<br>?&gt;
</div>
</div>
<hr>
