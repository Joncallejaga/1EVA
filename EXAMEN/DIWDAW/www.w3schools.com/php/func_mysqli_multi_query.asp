...
&nbsp;&nbsp;}<br><br>$sql = &quot;SELECT Lastname FROM Persons ORDER BY LastName;&quot;;<br>
 $sql .= &quot;SELECT Country FROM Customers&quot;;<br><br>// Execute multi query<br>if (mysqli_multi_query($con,$sql))<br>{<br>&nbsp; do<br>&nbsp;&nbsp;&nbsp; {<br>&nbsp;&nbsp;&nbsp; // Store first result set<br>&nbsp;&nbsp;&nbsp; if ($result=mysqli_store_result($con)) {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; // Fetch one and one row<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; while ($row=mysqli_fetch_row($result))<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; printf(&quot;%s\n&quot;,$row[0]);<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; }<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; // Free result set<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; mysqli_free_result($result);<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; }<br>&nbsp;&nbsp;&nbsp; }<br>&nbsp; while (mysqli_next_result($con));<br>}<br><br>mysqli_close($con);<br>
?&gt;
</div>
</div>
<hr>
