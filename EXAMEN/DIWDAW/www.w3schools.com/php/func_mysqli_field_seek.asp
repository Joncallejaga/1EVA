...
&nbsp;&nbsp;}<br><br>$sql=&quot;SELECT Lastname,Age FROM Persons ORDER BY Lastname&quot;;<br>
 <br>if ($result=mysqli_query($con,$sql))<br>&nbsp; {<br>&nbsp; // Get field info for 1st column (&quot;Lastname&quot;)<br>&nbsp; mysqli_field_seek($result,0);<br>&nbsp; $fieldinfo=mysqli_fetch_field($result);<br><br>&nbsp; printf(&quot;Name: %s\n&quot;,$fieldinfo-&gt;name);<br>&nbsp; printf(&quot;Table: %s\n&quot;,$fieldinfo-&gt;table);<br>&nbsp; printf(&quot;max. Len: %d\n&quot;,$fieldinfo-&gt;max_length);<br><br>&nbsp; // Free result set<br>&nbsp; mysqli_free_result($result);<br>}<br>
