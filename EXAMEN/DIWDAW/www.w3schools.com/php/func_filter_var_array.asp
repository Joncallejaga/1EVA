...
<table class="w3-table-all notranslate">
  ...
  <tr>
    <td>array</td>
    <td>Required. Specifies an array with string keys  containing the data to filter</td>
  </tr>
  <tr>
    <td>args</td>
    <td>Optional. Specifies an array of filter arguments. A  valid array key is a variable name and a valid value is  a filter ID, or an array specifying the filter,  flags and options.<p>This parameter can also be a single filter ID, if so,  all values in the input array are filtered by the specified filter<p>A filter ID can be an ID name (like FILTER_VALIDATE_EMAIL)  or an ID number (like 274)</td>
  </tr>
 </table>
<hr>
